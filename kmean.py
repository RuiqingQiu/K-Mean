...
def main():
    ...
    mn.test(0)
    index = 0
    for x in mn.train_images:
        if index == num_points-1:
            break
        p = Point(x)
        p.add_label(mn.train_labels[index])
        points.append(p)
        #print index
        index = index + 1
    # Cluster those data!
    ...
    # Print our clusters
    error_count = 0
    # A list keep track which cluster represent which digits
    clusters_num_lst = []
    for i,c in enumerate(clusters):
        ...
        for p in c.points:
            lst.append(p.label)
        tmp = Counter(lst)
        # get the most common digit that classified to this cluster
        num = tmp.most_common(1)[0][0]
        if num in clusters_num_lst:
            num = tmp.most_common(2)[1][0]
        clusters_num_lst.append(num)
        index = 0
        for p in c.points:
            print " Cluster: ", i, "\t"# Point :", p
            print mn.display(p.coords)
            print "Actual Label is : ", p.label
            print "Predicted Label is : ", clusters_num_lst[index]
            if num != int(p.label):
                error_count = error_count + 1
        index = index + 1

    print clusters_num_lst
    print "error count is ", error_count
    print "error rate is ", error_count/float(num_points)
